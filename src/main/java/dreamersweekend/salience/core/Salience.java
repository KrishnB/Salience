...
import dreamersweekend.salience.commons.PixelList;
import dreamersweekend.salience.commons.SalienceColor;
import dreamersweekend.salience.io.ImageReader;
...
public class Salience {
	...
	public Salience(File mImageFile,String mScenario, ImageComparisonType mImageCompType) {
	...
	public BufferedImage getImageDifferenceByPixelsOrRGB(ImageComparisonType imgCompType)
	{
		...
		if(imageDifferencePercentage==0.0){
			...
			if(imageDifferencePercentage==1000) {
				resizeImages();
			}
	...
	public static BufferedImage getDifferenceByRGB(BufferedImage im1, BufferedImage im2)
	{
	...
	public static BufferedImage getDifferenceByPixels(BufferedImage img1, BufferedImage img2) {
		SalienceColor color = new SalienceColor();
		final int w = img1.getWidth(), h = img1.getHeight(), highlight = color.getColor();
	...
	private void resizeImages() {
		if(mBaseImage!=null) {
			int imgWidth = mBaseImage.getWidth();
			int imgHeight = mBaseImage.getHeight();
			BufferedImage resizedImage = new BufferedImage(mBaseImage.getWidth(), mBaseImage.getHeight(), BufferedImage.TYPE_INT_RGB);
			Graphics2D g = resizedImage.createGraphics();
			g.drawImage(mCurrentImage, 0, 0, imgWidth, imgHeight, null);
			g.dispose();
			g.setComposite(AlphaComposite.Src);
			g.setRenderingHint(RenderingHints.KEY_INTERPOLATION,RenderingHints.VALUE_INTERPOLATION_BILINEAR);
			g.setRenderingHint(RenderingHints.KEY_RENDERING,RenderingHints.VALUE_RENDER_QUALITY);
			g.setRenderingHint(RenderingHints.KEY_ANTIALIASING,RenderingHints.VALUE_ANTIALIAS_ON);
			mCurrentImage = resizedImage;
		}
	}
}
